...
layout: page
title: "WrapLongURL"
description: "Shortens a long URL for email clients that truncate long hyperlinks. Outside a send the argument comes back byte for byte unchanged, whatever its length."
...
In an email send this yields a short platform link that forwards to the original address. Outside a send it does not: the argument is returned unchanged, however long it is. An **email _preview_** behaves the same as a CloudPage here — the long URL comes back unchanged, because a preview is not a send. Test the shortening in a real send, not on a page and not in a preview.
...
**Outside a send nothing is shortened.** A URL beyond the documented 975-character threshold is returned unchanged. This is the point worth remembering: a CloudPage will never show you the shortened form, so a page render is not a valid test of the feature.
...
- [Official reference](https://developer.salesforce.com/docs/marketing/marketing-cloud-ampscript/references/mc-ampscript-http/mc-ampscript-reference-http-wrap-long-url.html)
- [ampscript.guide](https://ampscript.guide/wraplongurl/)
