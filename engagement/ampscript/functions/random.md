...
layout: page
title: "Random"
description: "Returns a random whole number between two inclusive bounds. Covers the decimal bounds that the official docs allow but the runtime rejects."
...
Every returned value is a bare integer; a decimal point never appears. The value set is bounded only by the arguments you pass, so there is no fixed catalog of possible literals.
...
**Negative ranges work.** `Random(-2, -1)` produces only `-2` and `-1`, and produces both.

**Whole-number strings are accepted.** `Random("1", "2")` behaves identically to the numeric form, with both bounds reachable.
...
Treat the bounds as integers only, and round or truncate before the call. The finding is catalogued on [Differs from official docs](/engagement/differs-from-docs/#random-decimal-bounds-rejected).
...
- [Official reference](https://developer.salesforce.com/docs/marketing/marketing-cloud-ampscript/references/mc-ampscript-utilities/mc-ampscript-reference-utilities-random.html)
- [ampscript.guide](https://ampscript.guide/random/)
